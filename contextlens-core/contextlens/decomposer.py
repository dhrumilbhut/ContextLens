...
Updated in SDK Phase E session to match the production backend decomposer.
Adds compound-sentence splitting guidance and 3 few-shot examples targeting
sub-clause fragmentation (the Phase D validation audit's documented remaining gap).
This module is not used in production (backend/app/worker/decomposer.py is the
production path), but is kept in sync so contextlens-core stays usable as a
reference and for local testing.
"""
...
def decompose(llm_response: str) -> list[dict]:
    """Returns list of claim dicts: [{"claim_text": str, "is_refusal": bool}, ...]"""
